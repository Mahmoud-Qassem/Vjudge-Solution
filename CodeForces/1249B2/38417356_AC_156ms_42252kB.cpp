...
int x;
map<int,int>mp;
void dfs(int node,int cnt=1)
{
    if(not sTime[node])
        sTime[node]=cnt,x++;
        else if(not x)return ;
    for(auto&it:v[node])
    {
        if(sTime[it])
        {
            if(ans[it]==-1)
            ans[it]=cnt-sTime[it]+1,x--;
        }
            dfs(it,cnt+1);
    }
}
...
int32_t main()
{
    ...
    T
    {
        ...
        for(int i=0;i<n;++i)
        {
            if(help[i]!=i&&not visited[i])
            {
                dfs(i);
            }
        }
        for(auto&it:ans)if(it!=-1)cout<<it<<" ";else cout<<1<<" ";
        cout<<endl;
    }
